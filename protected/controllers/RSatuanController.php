<?php
class RSatuanController extends Controller
{
	/**
    ...
    */
    ...
    public function actionRead(){
            ...
            $total = RSatuan::model()->count(array(
				'condition'=>'deleted=0'
			));
			$limit=($limit+$start>$total)?($total-$start):$limit;
            $model = RSatuan::model()->findAll(array(
				'condition'=>'deleted=0',
				'limit'=>$limit, 
				'offset'=>$start
			));
	...
	{
		$model=RSatuan::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='rsatuan-form')
		{
