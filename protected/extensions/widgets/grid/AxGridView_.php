...
class AxGridView extends CGridView {
    ...
    public function init() {
        ...
        if ($this->deleteTopButton || !empty($this->topButtons))
    ...
    private function renderCreateButton() {
        if(!empty($this->createButtonOptions)){
            if(isset($this->createButtonOptions['popupwin']) && $this->createButtonOptions['popupwin']){
                ...
                $this->createButtonProperty['htmlOptions']['onclick'] = 'PopUpWinCreate'.$this->id.'("' . $this->createButtonUrl . '","'.$width.'","'.$height.'")';
            }
        }
        else
            $this->createButtonProperty['htmlOptions']['onclick'] = 'MM_goToURL'.$this->id.'("' . $this->createButtonUrl . '")';
    ...
    private function renderTopButtons() {
        ...
        foreach ($this->topButtons as $button) {
            ...
            if ($visible) {
                ...
                if (!empty($popupwin))
                    $this->createButtonProperty['htmlOptions']['onclick'] = 'PopUpWin'.$this->id.'("' . $button['url'][0] . '")';
                elseif (!empty($ajax))
                    $this->createButtonProperty['htmlOptions']['onclick'] = 'doAjaxAction'.$this->id.'("' . $button['url'][0] . '","'.$button['confirm'].'")';
                else
                    $this->createButtonProperty['htmlOptions']['onclick'] = 'MM_goToURL'.$this->id.'("' . $button['url'][0] . '")';
...
function doAjaxAction{$this->id}(url, msg){
    var arrVal = $.fn.yiiGridView.getChecked('{$this->id}','{$this->checkBoxClmId}');
    if(arrVal.length == 0){
        alert('Silahkan pilih minimal satu baris data');
        return false;
    ...
    var opt = {
        ...
        error:function(XHR) {
            showOnNotify(data.type, XHR.msg, {$this->notifyTime});
        }
    };
    $.fn.yiiGridView.update('{$this->id}', opt);
    return false;
}
function MM_goToURL{$this->id}(ulink)
{
    window.location=ulink;
}\n 
function PopUpWin{$this->id}(ulink)
{
    $.colorbox({iframe:true, overlayClose:false, scrolling:false, href:ulink,width:"{$this->popupWinWidth}", height:"{$this->popupWinHeight}"});
}\n 
function PopUpWinCreate{$this->id}(ulink, width, heigth)
{
    $.colorbox({iframe:true, overlayClose:false, scrolling:false, href:ulink, opacity:0, width:width, height:heigth});
}\n 
EOS;
