Ext.define('Esmk.view.TDataTanggalCutoff._grid', {
    extend: 'Ext.grid.Panel',
    alias: 'widget.tdatatanggalcutoffGrid',
	id:'tdatatanggalcutoffgridid',
    requires: [
        'Ext.toolbar.Paging',
    ...
    iconCls: 'icon-grid',
    title: 'Jadwal Tanggal Cut Off Data Dapodik',
    loadMask: true,
    ...
    initComponent: function() {
        var me = this;
		this.store=Ext.create('Esmk.store.TDataTanggalCutoff');
        Ext.applyIf(me, {
            columns: [
                {
                    xtype: 'rownumberer',
	...
	actionDbClick: function(dataview, record, item, index, e, options){
        var formTDataTanggalCutoff = Ext.create('Esmk.view.TDataTanggalCutoff._form');

        if (record) {

            formTDataTanggalCutoff.down('form').loadRecord(record);
    ...
    actionSearch: function(button) {
		...
		grid.getStore().getProxy().api.read='TDataTanggalCutoff/read/?q='+values;
    },
});
