Ext.define('Esmk.view.RBantuanPenerima._form', {
    //extend: 'Ext.form.Panel', //use this code for panel form
    extend: 'Ext.window.Window',
    alias: 'widget.rbantuanpenerimaForm',
    requires: ['Ext.form.Panel',
        'Ext.form.field.Text',
        'Ext.ux.DataTip',
        'Ext.data.*'
    ],
    title: 'Form Jenis Penerima',
    layout: 'fit',
    autoShow: true,
    width: 600,
    autoHeight:true,
	...
	actionSave: function(button) {
        ...
        if (values.id !='') {
			...
			Ext.getCmp('rbantuanpenerimagridid').getStore().load();
        } else {
            record = Ext.create('Esmk.model.RBantuanPenerima');
            record.set(values);
            Ext.getCmp('rbantuanpenerimagridid').getStore().add(record);
            isNewRecord = true;
			Ext.getCmp('rbantuanpenerimagridid').getStore().load();
        }
