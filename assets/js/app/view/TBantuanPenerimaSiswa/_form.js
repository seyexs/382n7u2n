Ext.define('Esmk.view.TBantuanPenerimaSiswa._form', {
    ...
    title: 'Siban - Form TBantuanPenerimaSiswa',
    layout: 'fit',
    autoShow: true,
    width: 600,
    autoHeight:true,
	...
	actionSave: function(button) {
        ...
        if (values.id !='') {
			...
			Ext.getCmp('tbantuanpenerimasiswagridid').getStore().load();
        } else {
            record = Ext.create('Esmk.model.TBantuanPenerimaSiswa');
            record.set(values);
            Ext.getCmp('tbantuanpenerimasiswagridid').getStore().add(record);
            isNewRecord = true;
			Ext.getCmp('tbantuanpenerimasiswagridid').getStore().load();
        }
