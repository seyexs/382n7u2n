Ext.define('Esmk.view.TBantuanPenerimaSiswa._grid', {
    extend: 'Ext.grid.Panel',
    alias: 'widget.tbantuanpenerimasiswaGrid',
	id:'tbantuanpenerimasiswagridid',
    requires: [
        'Ext.toolbar.Paging',
    ...
    iconCls: 'icon-grid',
    title: 'Siban - TBANTUANPENERIMASISWA',
    loadMask: true,
    ...
    initComponent: function() {
        var me = this;
		this.store=Ext.create('Esmk.store.TBantuanPenerimaSiswa');
		this.store.pageSize=500;
		this.store.getProxy().api.read='TBantuanPenerimaSiswa/read/?tbpid='+me.tbpid;
        Ext.applyIf(me, {
            columns: [
                {
                    xtype: 'rownumberer',
	...
	actionDbClick: function(dataview, record, item, index, e, options){
        var formTBantuanPenerimaSiswa = Ext.create('Esmk.view.TBantuanPenerimaSiswa._form');

        if (record) {

            formTBantuanPenerimaSiswa.down('form').loadRecord(record);
    ...
    actionSearch: function(button) {
		...
		grid.getStore().getProxy().api.read='TBantuanPenerimaSiswa/read/?q='+values;
    },
});
