Ext.define('Esmk.view.TBantuanLaporanPhoto._grid', {
    extend: 'Ext.grid.Panel',
    alias: 'widget.tbantuanlaporanphotoGrid',
	id:'tbantuanlaporanphotogridid',
    requires: [
        'Ext.toolbar.Paging',
    ...
    iconCls: 'icon-grid',
    title: 'Siban - TBANTUANLAPORANPHOTO',
    loadMask: true,
    ...
    initComponent: function() {
        var me = this;
		this.store=Ext.create('Esmk.store.TBantuanLaporanPhoto');
        Ext.applyIf(me, {
            columns: [
                {
                    xtype: 'rownumberer',
                ...
                {
                    dataIndex: 'keterangan_photo',
                    text: 'KETERANGAN_PHOTO',
                    flex:true,
	...
	actionDbClick: function(dataview, record, item, index, e, options){
        var formTBantuanLaporanPhoto = Ext.create('Esmk.view.TBantuanLaporanPhoto._form');

        if (record) {

            formTBantuanLaporanPhoto.down('form').loadRecord(record);
    ...
    actionSearch: function(button) {
		...
		grid.getStore().getProxy().api.read='TBantuanLaporanPhoto/read/?q='+values;
    },
});
