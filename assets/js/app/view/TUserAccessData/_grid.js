Ext.define('Esmk.view.TUserAccessData._grid', {
    extend: 'Ext.grid.Panel',
    alias: 'widget.tuseraccessdataGrid',
	id:'tuseraccessdatagridid',
    requires: [
        'Ext.toolbar.Paging',
    ...
    initComponent: function() {
        var me = this;
		this.store=Ext.create('Esmk.store.TUserAccessData');
        Ext.applyIf(me, {
            columns: [
                {
                    xtype: 'rownumberer',
	...
	actionDbClick: function(dataview, record, item, index, e, options){
        var formTUserAccessData = Ext.create('Esmk.view.TUserAccessData._form');

        if (record) {

            formTUserAccessData.down('form').loadRecord(record);
    ...
    actionSearch: function(button) {
		...
		grid.getStore().getProxy().api.read='TUserAccessData/read/?q='+values;
    },
});
